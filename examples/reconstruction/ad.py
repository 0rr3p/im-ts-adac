...
def run_detection(cfg):
    ...
    # 1. Gestione Label Map
    if LABEL_MAP_PATH.exists():
        label_map = joblib.load(LABEL_MAP_PATH)
        print(f"✅ Label map caricata: {LABEL_MAP_PATH}")
    else:
        label_map = {0: "Fuffa_Normale", 1: "Fuffa_Anomalo"}
        joblib.dump(label_map, LABEL_MAP_PATH)

    # 2. Gestione History (Dati)
    if DATA_XY_PATH.exists():
        history = joblib.load(DATA_XY_PATH)
        print(f"✅ history caricato: {DATA_XY_PATH}")
    else:
        fuffa_X1 = np.zeros(30).tolist()
        fuffa_X2 = np.ones(30).tolist()
        history = {"X": [fuffa_X1, fuffa_X2], "y": [0, 1]}
        joblib.dump(history, DATA_XY_PATH)
        print("⚠️ Inizializzata nuova history con dati fuffa.")

    # 3. Gestione Modello RF
    if RF_PATH.exists():
        rf_model = joblib.load(RF_PATH)
        print(f"✅ Modello RF caricato: {RF_PATH}")
    else:
        rf_model = RandomForestClassifier(n_estimators=100, random_state=42)
        rf_model.fit(history['X'], history['y'])
        joblib.dump(rf_model, RF_PATH)
        print("⚠️ Creato e addestrato nuovo modello RF sui dati fuffa.")
    ...
    print(f"DEBUG RF classes_: {rf_model.classes_}")
    print(f"DEBUG history classi uniche: {set(history['y'])}")
    ...
    for traj_id, windows in traj_windows.items():
        ...
        for win_num, w in enumerate(anom_windows):
            ...
            if win_key not in session_labels and rf_model:
                ...
                # 🌟 MODIFICA: Crea la stringa con tutte le probabilità formattate tra 0 e 1 (es. Classe: 0.XX)
                prob_list_str = []
                for class_idx, prob in sorted(zip(rf_model.classes_, probs), key=lambda x: -x[1]):
                    ...
                    prob_list_str.append(f"{nome}: {float(prob):.2f}")
                prob_rf_da_salvare = ", ".join(prob_list_str)
                ...
                if AUTO_CLASSIFY and best_prob >= RF_THRESHOLD:
            ...
            for i in range(3):
                ...
                for (t0, t1) in all_anom_intervals:
                    ax_mini[i].axvspan(t0, min(t1, T_total), color='slategray', alpha=0.30)
                t0_curr = win_starts[win_idx]
                ax_mini[i].axvspan(t0_curr, min(t0_curr + seq_len, T_total), color='black', alpha=0.70)
            ...
            if win_key not in session_labels:
                ...
                if AUTO_FALSE_POSITIVE:
                    ...
                    unified_records.append({
                        ...
                        'label': '', 'n_finestre_totali_traj': n_finestre_totali_traj,
                        'probabilita_rf': prob_rf_da_salvare  # 🌟 AGGIORNATO
                    })
                    continue
            ...
            if win_key in session_labels:
                ...
                if ans.isdigit():
                    ...
                    # Cerca se esiste già col nome
                    existing = next((k for k, v in label_map.items() if v == ans), None)
                    if existing is not None:
                        l_id = existing  # Classe già nota, riuso l'ID
                    else:
                        l_id = max(label_map.keys(), default=-1) + 1  # Nuova classe, ID sicuro
                        label_map[l_id] = ans
                ...
                unified_records.append({
                    ...
                    'probabilita_rf': prob_rf_da_salvare  # <--- NUOVA RIGA
                })
                ...
                # --- RIMOZIONE DATI FUFFA ---
                chiavi_fuffa = [k for k, v in label_map.items() if v in ["Fuffa_Normale", "Fuffa_Anomalo"]]
                if chiavi_fuffa:
                    classi_reali = set(history['y']) - set(chiavi_fuffa)
                    if len(classi_reali) >= 2:
                        print("🧹 Raggiunte due classi reali: elimino i dati fuffa iniziali!")
                        nuova_X, nuova_y = [], []
                        for x_val, y_val in zip(history['X'], history['y']):
                            if y_val not in chiavi_fuffa:
                                nuova_X.append(x_val)
                                nuova_y.append(y_val)
                        history['X'] = nuova_X
                        history['y'] = nuova_y
                        for k in chiavi_fuffa:
                            del label_map[k]

                # --- RIADDESTRAMENTO IN-LOOP ---
                if len(set(history['y'])) > 1:
