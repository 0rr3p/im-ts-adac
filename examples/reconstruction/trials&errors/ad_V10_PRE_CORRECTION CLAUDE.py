...
FAISS_SIMILARITY_THRESHOLD= 0.0001
RF_THRESHOLD= 0.90
...
def run_detection(cfg):
    ...
    for traj_id, windows in traj_windows.items():
        ...
        # Finestre anomale e parametri comuni
        anom_windows = [w for w in windows if w['mse'] > THRESHOLD]
        seq_len      = anom_windows[0]['target'].shape[0]
        n_anom       = len(anom_windows)
        feature_names = cfg.data.feature_cols

        # --- Ricostruzione traiettoria completa dal target delle finestre ---
        # Le finestre sono ordinate per win_idx; usiamo solo l'ultimo campione
        # di ogni finestra per evitare duplicati (stride può essere < seq_len).
        # Per la prima finestra prendiamo tutto, per le successive solo i punti
        # nuovi rispetto alla fine della finestra precedente.
        ...
        for prev_w, curr_w in zip(all_wins_sorted[:-1], all_wins_sorted[1:]):
            overlap = prev_w['win_idx']  # non usato direttamente
            # Calcoliamo quanti campioni sono già coperti dalla finestra precedente
            # confrontando gli ultimi valori (approssimazione robusta)
            prev_end = full_traj_parts[-1][-1]
            curr_start = curr_w['target'][0]
            # Cerchiamo il punto di giuntura: primo indice in curr che non è già in prev
            # Strategia: troviamo l'overlap confrontando la coda di full con l'inizio di curr
            full_so_far = np.concatenate(full_traj_parts, axis=0)
            n_full = len(full_so_far)
            n_curr = len(curr_w['target'])
            # Proviamo overlap da 0 a seq_len-1
            best_ov = 0
        ...
        # Mappa win_idx → intervallo [t_start, t_end) nella traiettoria completa
        # Ricostruiamo le posizioni di start di ogni finestra nella serie completa
        win_starts = {}
        cumlen = 0
        for k, w in enumerate(all_wins_sorted):
            if k == 0:
                ...
                prev_w2 = all_wins_sorted[k - 1]
                # overlap già calcolato sopra; lo ricalcoliamo velocemente
        ...
        # Intervalli anomali nella traiettoria completa
        anom_intervals = [(win_starts[w['win_idx']], win_starts[w['win_idx']] + seq_len)
                          for w in anom_windows]

        # --- Layout figura: 3 righe mini (overview) + 3 righe dettaglio ---
        title_suffix = f"({n_anom} finestre anomale)" if n_anom > 1 else ""
        fig = plt.figure(figsize=(max(12, 4 * n_anom), 11))
        # GridSpec: 3 mini + spazio + 3 dettaglio
        import matplotlib.gridspec as gridspec
        gs = gridspec.GridSpec(7, 1, figure=fig,
                               height_ratios=[1, 1, 1, 0.3, 2, 2, 2],
                               hspace=0.05)

        # --- Mini overview (righe 0-2) ---
        ...
        for i in range(3):
            ...
            # Evidenzia le finestre anomale in rosso semi-trasparente
            for (t0, t1) in anom_intervals:
                ax_mini[i].axvspan(t0, min(t1, T_total), color='red', alpha=0.25)
        ...
        ax_mini[0].set_title(f"ID: {traj_id} — Traiettoria completa  {title_suffix}", fontsize=9)
        ...
        # --- Dettaglio finestre anomale concatenate (righe 4-6) ---
        ...
        for i in range(3):
            ax_det[i].plot(target_concat[:, i], color='tab:blue',   label='Input reale')
            ax_det[i].plot(output_concat[:, i], color='tab:orange', linestyle='--', label='Ricostruito')
            for ww in range(1, n_anom):
                ax_det[i].axvline(x=ww * seq_len - 0.5, color='gray', linestyle=':', alpha=0.6)
        ...
        ax_det[0].set_title("Dettaglio finestre anomale", fontsize=9)
