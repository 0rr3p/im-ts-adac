...
class TimeSeriesDataset(object):
    def __init__(
        ...
        self.k= k
        self.first_n_ignore = first_n_ignore
    ...
    def preprocess_with_loaded_scaler(self):
        ...
        return [], scaled_data, traj_ids  # ← restituisci gli id
    ...
    def frame_series(self, traj_list,traj_ids_list):
        """
        ...
        """
        features, y_hist, targets, ids, starts = [], [], [], [], []

    
        max_stride = int(self.seq_length * (1 - self.overlap))  # e.g. 80 for seq=100, overlap=0.2
            ...
            for i in start_indices:
                ...
                if self.task == "reconstruction":
                    if i == 0:
                        ...
                        first_val = torch.FloatTensor(X[0:1, :])
                        rest  = torch.FloatTensor(X[0: self.seq_length - 1, :])
                        yh    = torch.cat([first_val, rest], dim=0)
                    else:
        ...
        return TensorDataset(torch.cat(features), torch.cat(y_hist), torch.cat(targets), 
                     torch.tensor(ids), torch.tensor(starts))
    # ------------------------------------------------------------------
    # Public API
    # ------------------------------------------------------------------
    ...
    def get_loaders(self):
        ...
        train_iter = DataLoader(
            ...
            generator=g_train,      # ← generator dedicato
        )
        test_iter = DataLoader(
            ...
            drop_last=False,
            worker_init_fn=seed_worker,
            generator=g_test,       # ← generator dedicato
        )
