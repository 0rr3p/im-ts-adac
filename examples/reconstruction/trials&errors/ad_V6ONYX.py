...
# --- INIZIO MODIFICA: ESPORTAZIONE ONNX ---
    onnx_path = FAISS_DIR / f"autoenc_model_joint{SELECTED_JOINT}.onnx"
    
    if not onnx_path.exists():
        print(f"\n🔄 Esportazione del modello PyTorch in formato ONNX in corso...")
        
        try:
            sample_batch = next(iter(test_loader))
            dummy_features, dummy_y_hist, _, _ = [b.to(device) for b in sample_batch]
            
            # OPZIONALE: Se in futuro vorrai fare inferenza su 1 singola finestra alla volta, 
            # decommenta queste due righe per forzare l'ONNX a lavorare con batch_size=1
            # dummy_features = dummy_features[0:1]
            # dummy_y_hist = dummy_y_hist[0:1]
            
            print("Tracciamento JIT in corso...")
            # Strict=False aiuta a silenziare alcuni problemi legati a dizionari/tuple nel JIT
            traced_model = torch.jit.trace(model, (dummy_features, dummy_y_hist), strict=False)
            
            torch.onnx.export(
                traced_model,                               
                (dummy_features, dummy_y_hist),
                str(onnx_path),
                export_params=True,
                opset_version=14,                           
                do_constant_folding=True,                   
                input_names=['features', 'y_hist'],         
                output_names=['output']
                # RIMOSSO COMPLETAMENTE: dynamic_axes
            )
            print(f"✅ Modello ONNX esportato con successo in: {onnx_path}")
        except Exception as e:
            print(f"❌ Errore durante l'esportazione ONNX: {e}")
    else:
        print(f"\n✅ Modello ONNX già presente in: {onnx_path}")
    # --- FINE MODIFICA ---
