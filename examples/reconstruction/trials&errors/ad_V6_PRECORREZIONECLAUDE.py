...
def run_detection(cfg):
    ...
    with torch.no_grad():
        ...
        if true_anomalies_indices:
            ...
            session_labels = {}
            all_residuals = np.concatenate(anomaly_residuals_list, axis=0)  # shape: (N_anomalie_totali, 60, 3)

            for idx in true_anomalies_indices:
                info = anomaly_info[idx]
                residuo_singolo = all_residuals[idx] # Shape (60, 3)
                traj_id = int(info['id'])
                ...
                if traj_id in session_labels:
                    ...
                    if rf_model:
                        ...
                        if best_prob >= RF_THRESHOLD:
                            ...
                            continue  # Salta passaggi successivi
        ...
        if anomaly_info:
            ...
            if indices_da_salvare:
                ...
                final_residuals = np.concatenate([anomaly_residuals_list[0][i:i+1] for i in indices_da_salvare], axis=0)
                final_embeddings = np.concatenate([anomaly_embeddings_list[0][i:i+1] for i in indices_da_salvare], axis=0)
