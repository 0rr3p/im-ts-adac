...
class Encoder(nn.Module):
    ...
    def forward(self, input_data: torch.Tensor):
        ...
        input_encoded = torch.zeros(input_data.size(0), self.seq_len, self.hidden_size).to(device) ##tolto variable e aggiunto to device
...
class Decoder(nn.Module):
    ...
    def forward(self, _, y_hist: torch.Tensor):
        ...
        inp = y_hist[:, 0].unsqueeze(0).unsqueeze(2)
    
        for t in range(self.seq_len):
            lstm_out, (h_t, c_t) = self.lstm(inp, (h_t, c_t))
            out = self.fc(lstm_out.squeeze(0))      # (bs, out_feats)
            outputs.append(out)
            inp = out.detach().unsqueeze(0)         # ← output precedente come nuovo input
            # .detach() evita che il gradiente scorra attraverso i passi auto-regressivi
