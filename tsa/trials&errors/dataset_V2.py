...
class TimeSeriesDataset(object):
    ...
    def frame_series(self, traj_list, min_overlap: float = 0.4):
        """
        ...
        for X in traj_list:
            ...
            # Usable length (leaving room for prediction window)
            usable_len = nb_obs - self.prediction_window
    
            # Minimum number of windows needed to cover usable_len with max_stride
            n_windows = math.ceil((usable_len - self.seq_length) / max_stride) + 1
    
            # Recompute actual stride to distribute windows uniformly
            if n_windows > 1:
            ...
            for i in start_indices:
                # Skip if y_hist would go out of bounds (i == 0 edge case)
                if i == 0:
                    continue
                ...
                if self.task == "reconstruction":
                    ...
                    tgt = torch.FloatTensor(
                        X[i + self.seq_length: i + self.seq_length + self.prediction_window, :]
                    )
                    targets.append(tgt)
    
        features_var = torch.cat(features)
        y_hist_var = torch.cat(y_hist)
        target_var = torch.cat(targets)
