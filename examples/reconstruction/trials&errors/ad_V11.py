...
def run_detection(cfg):
    ...
    # Stato sessione
    ids_to_ignore  = set()   # falsi positivi confermati in questa sessione (traj_id)
    ...
    for traj_id, windows in traj_windows.items():
        ...
        for win_num, w in enumerate(anom_windows):
            ...
            for i in range(3):
                ax_det[i].plot(w['target'][:, i], color='tab:blue',   label='Input reale')
                ax_det[i].plot(w['output'][:, i], color='tab:orange', linestyle='--', label='Ricostruito')
            ...
            ax_det[0].set_title(
                f"Dettaglio finestra {win_num + 1} (win_idx={win_idx}) | MSE={w['mse']:.6f}", fontsize=9)
