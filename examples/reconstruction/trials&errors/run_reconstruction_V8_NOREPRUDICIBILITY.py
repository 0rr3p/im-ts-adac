import hydra
import torch
import torch.nn as nn
from hydra.utils import instantiate

from tsa import AutoEncForecast, train, evaluate
from tsa.utils import load_checkpoint
import numpy as np
import random
import os

def set_seed(seed: int = 42):
    # Python native
    random.seed(seed)
    
    # OS environment
    os.environ['PYTHONHASHSEED'] = str(seed)
    
    # NumPy
    np.random.seed(seed)
    
    # PyTorch (CPU e GPU)
    torch.manual_seed(seed)
    torch.cuda.manual_seed(seed)
    torch.cuda.manual_seed_all(seed) # per multi-GPU
    
    # Determinismo negli algoritmi di convoluzione/cuDNN
    torch.backends.cudnn.deterministic = True
    torch.backends.cudnn.benchmark = False
    

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')


@hydra.main(version_base="1.1",config_path="./", config_name="config")
def run(cfg):
    set_seed(42) 
    print(f"--- Avvio Training per Giunto {cfg.joint_id} ---")
    ts = instantiate(cfg.data)
    train_iter, test_iter, nb_features = ts.get_loaders()
    ts.save_scaler(f"scaler_joint{cfg.joint_id}.pkl")
    
    # ── STEPS DIAGNOSTICS ──────────────────────────────────────────
    steps_per_epoch = len(train_iter)
    total_steps = steps_per_epoch * cfg.training.num_epochs
    print(f"\n Steps per epoca : {steps_per_epoch}")
    print(f" Epoche config   : {cfg.training.num_epochs}")
    print(f" Steps TOTALI    : {total_steps}  ← usa questo come riferimento\n")
    # ───────────────────────────────────────────────────────────────


    model = AutoEncForecast(cfg.training, input_size=nb_features).to(device)
    criterion = nn.MSELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=cfg.training.lr)

    if cfg.general.do_eval and cfg.general.get("ckpt", False):
        model, _, loss, epoch = load_checkpoint(cfg.general.ckpt, model, optimizer, device)
        evaluate(test_iter, loss, model, cfg, ts)
    elif cfg.general.do_train:
        train(train_iter, test_iter, model, criterion, optimizer, cfg, ts)


if __name__ == "__main__":
    run()
