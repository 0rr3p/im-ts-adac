...
def add_fp_dual_index(emb, raw_index, centroids_index, metadata,
    ...
    # 1. Cerca il centroide più vicino
    if centroids_index.ntotal > 0:
        distances, indices = centroids_index.search(emb_f32, 1)
        dist        = float(distances[0, 0])
        nearest_idx = int(indices[0, 0])
    else:
        dist, nearest_idx = float('inf'), -1

    # 2. Merge o nuovo cluster
    if dist < threshold and nearest_idx >= 0:
        n            = metadata['cluster_counts'][nearest_idx]
        old_centroid = centroids_index.reconstruct(nearest_idx).copy()
        new_centroid = old_centroid + (emb_f32[0] - old_centroid) / (n + 1)

        # Ricostruisce centroids_index aggiornando solo il centroide modificato
        ...
        metadata['cluster_counts'][nearest_idx] = n + 1
        assigned_cluster = nearest_idx
        print(f"   🔄 Merge nel cluster {nearest_idx} (dist={dist:.4f}, n={n+1})")
    else:
        centroids_index.add(emb_f32)
        assigned_cluster = len(metadata['cluster_counts'])
        metadata['cluster_counts'].append(1)
        print(f"   ➕ Nuovo cluster #{assigned_cluster} (dist={dist:.4f})")

    # 3. Aggiungi il raw e aggiorna metadata
...
def add_fp_dual_index(emb, raw_index, centroids_index, metadata,
    ...
    # Crea sempre un nuovo cluster (dist >= threshold garantita a monte)
    centroids_index.add(emb_f32)
    assigned_cluster = len(metadata['cluster_counts'])
...
def run_detection(cfg):
    ...
    # Dizionario centrale: chiave = (traj_id, win_idx)
    # -----------------------------------------------------------------------
    ...
    session_summary = []  # riepilogo finale post-sessione
    ...
    for traj_id, windows in traj_windows.items():
        ...
        # Log delle finestre saltate per FP VERSIONE VECCHIA
        ...
        for w in fp_skipped:
            ...
            session_summary.append({'Joint_ID': SELECTED_JOINT, 'id': traj_id, 'finestra': w['win_idx'], 'mse': w['mse'], 'esito': 'FP_noto', 'label': ''})

        fp_skipped = [w for w in windows if w['mse'] > THRESHOLD and w['is_known_fp']]
        for w in fp_skipped:
            print(f"⏭️  ID {traj_id} finestra {w['win_idx']}: riconosciuta come FP da FAISS, ignorata.")
            
            # ← AGGIUNTA: aggiorna il centroide più vicino con media mobile
            ...
            # Ricostruisce l'indice aggiornando solo il centroide modificato
            all_centroids = np.stack([
                new_centroid if i == nearest_idx else centroids_index.reconstruct(i)
                for i in range(centroids_index.ntotal)
            ]).astype('float32')
       ...
       # DOPO
        ...
        for win_num, w in enumerate(anom_windows):
            ...
            classified_by_rf = False             # ← aggiungi qui
            if win_key not in session_labels and rf_model:
                ...
                if best_prob >= RF_THRESHOLD:
                    ...
                    classified_by_rf = True          # ← aggiungi qui
            ...
            if win_key not in session_labels:
                ...
                if AUTO_FALSE_POSITIVE:
                    ...
                    raw_index, centroids_index, fp_metadata = add_fp_dual_index(
                        ...
                        raw_path, centroids_path, meta_path,
                        threshold=FAISS_SIMILARITY_THRESHOLD, enc_h=enc_h
                    )
                    plt.close(fig)
                    session_summary.append({'Joint_ID': SELECTED_JOINT, 'id': traj_id, 'finestra': win_idx, 'mse': w['mse'], 'esito': 'FP_nuovo_auto', 'label': ''})
                ...
                if ans.strip().lower() == 'false':
                    ...
                    session_summary.append({'Joint_ID': SELECTED_JOINT, 'id': traj_id, 'finestra': win_idx, 'mse': w['mse'], 'esito': 'FP_nuovo', 'label': ''})
                    continue  # Passa alla finestra successiva
                ...
                if ans.lower() != 's' and ans.strip():
                    ...
                    session_summary.append({'Joint_ID': SELECTED_JOINT, 'id': traj_id, 'finestra': win_idx, 'mse': w['mse'], 'esito': 'saltata', 'label': ''})
            else:
                plt.close(fig)
            ...
            if win_key in session_labels:
                ...
                session_summary.append({'Joint_ID': SELECTED_JOINT, 'id': traj_id,
                    'finestra': win_idx, 'mse': w['mse'],
                    'esito': esito, 'label': label_map.get(l_id, ans)})
    ...
    # -----------------------------------------------------------------------
    # 9. CSV GLOBALE DI VALUTAZIONE
    # -----------------------------------------------------------------------
    dataset_name = Path(cfg.data.data_path).stem  # prende il nome del file senza estensione
    
    global_records = []
    for traj_id, windows in traj_windows.items():
        n_tot       = len(windows)
        n_pre_faiss = sum(1 for w in windows if w['mse'] > THRESHOLD)
        n_faiss_filtered = sum(1 for w in windows if w['mse'] > THRESHOLD and w.get('is_known_fp', False))
        n_post_faiss     = n_pre_faiss - n_faiss_filtered
        traj_flaggata    = n_post_faiss > 0
        ...
        global_records.append({
            ...
            'n_finestre_anomale_pre_faiss':  n_pre_faiss,
            'n_finestre_filtrate_faiss':     n_faiss_filtered,
            'n_finestre_anomale_post_faiss': n_post_faiss,
            'traj_flaggata':            traj_flaggata,
            'mu':                       mu,
            'sigma':                    sigma,
            'threshold':                THRESHOLD,
        })
    
    global_csv_path = Path(__file__).resolve().parent / "global_eval.csv"
    file_exists_global = os.path.exists(global_csv_path)
    pd.DataFrame(global_records).to_csv(
        global_csv_path, mode='a', index=False,
        header=not file_exists_global, sep=';', decimal=','
    )
    print(f"📊 global_eval.csv aggiornato con {len(global_records)} traiettorie (dataset={dataset_name}, joint={SELECTED_JOINT}).")


    if session_summary:
        csv_summary_path = Path(__file__).resolve().parent / "session_summary.csv"
        file_exists_summary = os.path.exists(csv_summary_path)
        pd.DataFrame(session_summary).to_csv(
            csv_summary_path, mode='a', index=False,
            header=not file_exists_summary, sep=';', decimal=','
        )
        print(f"📊 Riepilogo sessione salvato in 'session_summary.csv'.")
