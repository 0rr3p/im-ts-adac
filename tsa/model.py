...
class TemporalAttnPooling(nn.Module):
    def __init__(self, enc_h):
        super().__init__()
        # Un singolo vettore query appreso
        self.query = nn.Parameter(torch.randn(1, 1, enc_h))
        n_heads = max(1, enc_h // 8)
        self.attn  = nn.MultiheadAttention(enc_h, num_heads=n_heads, batch_first=True)

    def forward(self, input_encoded):
        # input_encoded: (B, seq_len, enc_h)
        q = self.query.expand(input_encoded.size(0), -1, -1)  # (B, 1, enc_h)
        z, _ = self.attn(q, input_encoded, input_encoded)
        # z: (B, 1, enc_h) → (B, enc_h)
        return z.squeeze(1) # bottleneck vettoriale + pesi temporali
###########################################################################
################################ DECODERS #################################
###########################################################################
...
class Decoder(nn.Module):
    def __init__(self, config):
        ...
        # Espande il bottleneck (B, enc_h) → (B, seq_len, dec_h)
...
class AutoEncForecast(nn.Module):
    def __init__(self, config, input_size):
        ...
        self.encoder = AttnEncoder(config, input_size).to(device) \
            if config['input_att'] else Encoder(config, input_size).to(device)
        
        # temporal_att=True  → pooling con attention learnable
        # temporal_att=False → mean pooling semplice (ma bottleneck c'è sempre)
        self.temporal_pool = TemporalAttnPooling(enc_h).to(device) \
            if config['temporal_att'] else None

        # Un solo decoder: parte sempre da (B, enc_h) → (B, seq_len, out_f)
        self.decoder = Decoder(config).to(device)

    def forward(self, encoder_input, y_hist=None, return_attention=False):
        input_attentions, encoder_output = self.encoder(encoder_input)
        # encoder_output: (B, seq_len, enc_h)

        if self.temporal_pool is not None:
            z = self.temporal_pool(encoder_output)   # (B, enc_h) — attention pooling
        else:
            z = encoder_output.mean(dim=1)           # (B, enc_h) — mean pooling

        outputs = self.decoder(z)                    # (B, seq_len, out_f)

        if return_attention:
            return outputs, input_attentions
        return outputs