    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            if is_anomalous.any():
                ...
                target_anomali = target[is_anomalous].cpu().numpy()
                output_anomali = output[is_anomalous].cpu().numpy()
                ...
                for i_id, i_err, i_emb, i_res, i_tgt, i_out in zip(
                        ids_anomali, err_anomali, emb_anomali, delta_np, target_anomali, output_anomali):
                    anomaly_info.append({
                        'id':       i_id,
                        'mse':      i_err,
                        'emb':      i_emb,
                        'residual': i_res,
                        'target':   i_tgt,   # shape (seq_len, features)
                        'output':   i_out,   # shape (seq_len, features)
                    })
        ...
        if true_anomalies_indices:
            ...
            for idx in true_anomalies_indices:
                ...
                if traj_id in session_labels:
                    ...
                    # DOPO (segnale reale + ricostruzione tratteggiata)
                    window_counter[traj_id] = window_counter.get(traj_id, 0) + 1
                    win_num = window_counter[traj_id]
                    
                    target_np = info['target']   # (seq_len, features)
                    output_np = info['output']   # (seq_len, features)
                    
                    fig, axs = plt.subplots(3, 1, figsize=(10, 8), sharex=True)
                    feature_names = cfg.data.feature_cols
                    for i in range(3):
                        axs[i].plot(target_np[:, i], color='tab:blue', label='Input reale')
                        axs[i].plot(output_np[:, i], color='tab:orange', linestyle='--', label='Ricostruito')
                        axs[i].set_ylabel(feature_names[i])
                        axs[i].legend(loc='upper right', fontsize=8)
                        axs[i].grid(True, alpha=0.3)
                    
                    axs[0].set_title(f"ID: {traj_id} — Window {win_num}")
