...
class Decoder(nn.Module):
    ...
    def forward(self, _, y_hist: torch.Tensor):
        ...
        for t in range(self.seq_len):
            ...
            outputs.append(self.fc(lstm_out.squeeze(0)))  # (bs, out_feats)
    
        return torch.stack(outputs, dim=1)  # (bs, seq_len, out_feats)
...
class AttnDecoder(nn.Module):
    ...
    def forward(self, input_encoded, y_history):
        ...
        for t in range(self.seq_len):
            ...
            y_tilde = self.fc(torch.cat((context, y_history[:, t].to(device)), dim=1))
            self.lstm.flatten_parameters()
            _, (h_t, c_t) = self.lstm(y_tilde.unsqueeze(0), (h_t, c_t))
            outputs.append(self.fc_out(torch.cat((h_t[0], context), dim=1)))  # (bs, out_feats)
