...
def train(train_iter, test_iter, model, criterion, optimizer, config, ts):
    ...
    start_saving_from= 1-config.training.from_last_steps
    start_saving_best = int(start_saving_from * total_steps) 
    # --------------------------

    
    sched_name = config.training.scheduler
    lr_min = config.training.lr * 1e-2          # es. 3e-3 * 0.01 = 3e-5
    T_cycle = config.training.num_epochs // 3  # es. 99 // 3 = 33
    ...
    else:  # "step" — comportamento originale
        scheduler = torch.optim.lr_scheduler.StepLR(
            optimizer,
            step_size=config.training.lr_decay_every_n_epoch,
            gamma=0.5
        )
                    ...
                    if config.general.eval_during_training:
                        ...
                        if global_step >= start_saving_best and current_custom_score < best_custom_score:
                            ...
                            torch.save({
                                ...
                                "temporal_pool_state_dict": model.temporal_pool.state_dict() if model.temporal_pool else None,  # ← AGGIUNTO
                            ...
                            # --- SALVATAGGIO BEST MODEL RESULTS (corretto per grid search) ---
                            csv_path = os.path.join(config.general.output_dir, "best_model_results.csv")
                            file_exists = os.path.isfile(csv_path)
                            
                            # Prepara riga con metriche + iperparametri del run corrente
                            csv_row = {
                                # Identificatori del run (fondamentali per la grid search)
                                "run_dir":        config.general.output_dir,
                                "joint_id":       config.joint_id,
                                "input_att":      config.training.input_att,
                                "temporal_att":   config.training.temporal_att,
                                "hidden_size":    config.training.hidden_size_encoder,
                            ...
                            # Aggiunge eventuali metriche extra da results
                            ...
                            with open(csv_path, mode='w', newline='', encoding='utf-8') as f:
                                ...
                                writer.writeheader()
                                # csv.DictWriter non gestisce la virgola decimale: formattiamo a mano i float
                                italian_row = {
                                    k: str(v).replace('.', ',') if isinstance(v, float) else v
                                    for k, v in csv_row.items()
                                }
                                writer.writerow(italian_row)  
