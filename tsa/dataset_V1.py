...
class TimeSeriesDataset(object):
    ...
    def frame_series(self, traj_list):
        """
        Build sliding windows across all trajectories.
        Each trajectory is windowed independently so windows never
        cross trajectory boundaries.

        Returns a TensorDataset of (features, y_hist, target).
        """
        features, y_hist, targets = [], [], []

        for X in traj_list:
            nb_obs = X.shape[0]

            # Need at least seq_length + prediction_window + 1 steps
            min_len = self.seq_length + self.prediction_window + 1
            if nb_obs < min_len:
                continue

            for i in range(1, nb_obs - self.seq_length - self.prediction_window):
                feat = torch.FloatTensor(X[i: i + self.seq_length, :])          # (seq_len, n_feat)
                features.append(feat.unsqueeze(0))

                if self.task == "reconstruction":
                    # y_hist: previous step features (for teacher forcing / context)
                    yh = torch.FloatTensor(X[i - 1: i + self.seq_length - 1, :])
                    y_hist.append(yh.unsqueeze(0))
                    # target: next prediction_window steps
                    tgt = torch.FloatTensor(X[i + self.seq_length: i + self.seq_length + self.prediction_window, :])
                    targets.append(tgt)

        features_var = torch.cat(features)          # (N, seq_len, n_feat)
        y_hist_var = torch.cat(y_hist)              # (N, seq_len, n_feat)
        target_var = torch.cat(targets)             # (N, prediction_window, n_feat)
    ...
    def get_loaders(self):
        ...
        train_dataset = self.frame_series(train_scaled)
        test_dataset = self.frame_series(test_scaled)
